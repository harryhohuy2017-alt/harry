...
export function createWorld(width=32,height=18){
  const world=[];
...
export function blockAt(world,x,y){
  return world.find(b=>b.x===x*30&&b.y===y*30)||null;
}

export function setBlock(world,x,y,type){
  const block=blockAt(world,x,y);
  if(!block){world.push({x:x*30,y:y*30,type});return true;}
  block.type=type;return true;
}

export function removeBlock(world,x,y){
  const i=world.findIndex(b=>b.x===x*30&&b.y===y*30);
  if(i<0)return null;
  return world.splice(i,1)[0];
}

export function createInventory(){return {grass:0,dirt:0,stone:0,sand:0,stick:0,plank:0};}
...
export function craft(inventory,recipe){
  ...
  for(const [item,count] of Object.entries(recipe.input))inventory[item]-=count;
  for(const [item,count] of Object.entries(recipe.output))addItem(inventory,item,count);
  return true;
}
